...
class _BlockPropertiesEditorState extends State<BlockPropertiesEditor> {
  ...
  List<Widget> _buildContentTab(LocalizationCubit loc, LandingPageBuilderCubit cubit, Map<String, dynamic> block, String type) {
    final List<Widget> list = [];

    // All text fields and specific editors
    list.add(
      FormGroup(
    ...
    // Block-specific text/image editors
    switch (type) {
      case 'hero':
      case 'hero_saas':
        list.addAll([
          ...
          FormGroup(
            ...
            helperText: "مثال: https://wa.me/xxx أو /contact",
            child: CustomTextField(
        ...
        break;
      case 'logo_header':
        list.add(LogoHeaderEditor(
          cubit: cubit,
          block: block,
          index: widget.index,
          getController: _getController,
          getFocusNode: _getFocusNode,
          pickImage: (c, idx, {itemIndex, itemKey}) => _pickMedia(c, idx, itemKey: itemKey, itemIndex: itemIndex),
          pickAndUploadImage: (c, idx, {itemIndex, itemKey}) => _pickMedia(c, idx, itemKey: itemKey, itemIndex: itemIndex),
        ));
        break;
      case 'multi_step_lead_form':
        list.add(MultiStepFormEditor(
          cubit: cubit,
          block: block,
          index: widget.index,
          getController: _getController,
      ...
      case 'whatsapp':
        list.addAll([
          ...
          const SizedBox(height: 16),
          FormGroup(
            label: loc.translate('initial_message'),
            child: CustomTextField(
      ...
      case 'animated_counter':
        list.add(AnimatedCounterEditor(
          cubit: cubit,
          block: block,
          index: widget.index,
          getController: _getController,
  ...
  List<Widget> _buildStyleTab(LocalizationCubit loc, LandingPageBuilderCubit cubit, Map<String, dynamic> block, String type) {
    return [
      FormGroup(
              ...
              items: [
                ...
                ...LandingPageTheme.availableFonts.map((font) {
                  return DropdownMenuItem<String>(
                    value: font['family'],
                    child: Text(font['family']!, style: const TextStyle(fontWeight: FontWeight.bold)),
                  );
                }),
              ],
              onChanged: (val) => cubit.updateBlockProperty(widget.index, 'fontFamily', val),
            ),
          ),
        ),
      ),
          ...
          children: [
            Slider(
              ...
              onChanged: (val) {
                ...
                cubit.updateBlockProperty(widget.index, 'bg_overlay_opacity', val);
              },
            ),
            Row(
              children: [
                _buildColorPreset(cubit, widget.index, '#000000', Colors.black, isSelected: (block['bg_overlay_color'] ?? '') == '#000000'),
                const SizedBox(width: 8),
                _buildColorPreset(cubit, widget.index, '#FFFFFF', Colors.white, isSelected: (block['bg_overlay_color'] ?? '') == '#FFFFFF'),
              ],
            ),
          ],
        ),
      ...
      const SizedBox(height: 16),
      FormGroup(
        label: loc.translate('vertical_padding'),
        child: Slider(
          value: ((block['vertical_padding'] ?? 60.0) as num).toDouble(),
          min: 0.0,
          max: 200.0,
          divisions: 20,
          activeColor: AppColors.secondary,
          onChanged: (val) => cubit.updateBlockProperty(widget.index, 'vertical_padding', val),
        ),
      ),
    ];
  }

  List<Widget> _buildAdvancedTab(LocalizationCubit loc, LandingPageBuilderCubit cubit, Map<String, dynamic> block, String type) {
    return [
      SwitchListTile(
        title: Text(loc.translate('visible')),
        value: block['is_visible'] ?? true,
        activeThumbColor: AppColors.secondary,
        onChanged: (val) => cubit.updateBlockProperty(widget.index, 'is_visible', val),
      ),
      const SizedBox(height: 16),
      if (type == 'gallery') ...[
        Text(loc.translate('display_mode'), style: AppTypography.bodyMedium.copyWith(fontWeight: FontWeight.bold)),
        ...
        Row(
          children: [
            _buildAdvancedOption(
              ...
              onTap: () => cubit.updateBlockProperty(widget.index, 'display_mode', 'grid'),
            ),
            const SizedBox(width: 8),
            _buildAdvancedOption(
              ...
              onTap: () => cubit.updateBlockProperty(widget.index, 'display_mode', 'carousel'),
            ),
          ],
        ),
      ],
      if (type == 'basic_section') ...[
        _buildDropdown(loc.translate('layout_direction'), 'layout_direction', ['column', 'row'], (val) => cubit.updateBlockProperty(widget.index, 'layout_direction', val)),
        _buildSlider(loc.translate('spacing'), 'spacing', 0, 100, (val) => cubit.updateBlockProperty(widget.index, 'spacing', val)),
      ],
    ];
  }

  Widget _buildAdvancedOption({required bool isSelected, required IconData icon, required String label, required VoidCallback onTap}) {
    return Expanded(
      child: InkWell(
        onTap: onTap,
        borderRadius: BorderRadius.circular(12),
        child: AnimatedContainer(
          ...
          child: Column(
            children: [
              ...
              Text(label, style: AppTypography.caption.copyWith(color: isSelected ? AppColors.secondary : AppColors.textSecondary, fontWeight: isSelected ? FontWeight.bold : FontWeight.normal)),
            ],
          ),
        ),
      ),
  ...
  Widget build(BuildContext context) {
    ...
    if (_activeTab == 0) {
      ...
      activeWidgets = _buildStyleTab(loc, cubit, block, type);
    } else {
      activeWidgets = _buildAdvancedTab(loc, cubit, block, type);
    }
        ...
        children: [
          Row(
            ...
            children: [
              ...
              TextButton(onPressed: widget.onDone, child: Text(loc.translate('close'), style: AppTypography.button.copyWith(color: AppColors.secondary))),
            ],
          ),
          const SizedBox(height: 24),
          ...
          Container(
            ...
            child: Row(
              children: [
                _buildTabButton(0, loc.translate('content'), Icons.edit_note_rounded),
                _buildTabButton(1, loc.translate('style'), Icons.palette_rounded),
                _buildTabButton(2, loc.translate('advanced'), Icons.settings_suggest_rounded),
              ],
            ),
          ),
          const SizedBox(height: 24),
  ...
  Widget _buildSlider(String label, String key, double min, double max, Function(double) onChanged) {
    ...
    return Column(
      ...
      children: [
        ...
        Slider(value: value.clamp(min, max), min: min, max: max, activeColor: AppColors.secondary, onChanged: onChanged),
      ],
    );
  }
}
